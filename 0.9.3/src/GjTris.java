...
public class GjTris extends JFrame {
	...
	public GjTris() {
		super("GjTris 0.9.3");
		setMarks("O", "X"); // It will be overrided by main method when GjTris will be istanced.
		enableSound = false;
		loadLanguage();
					...
					if(!isEnded) {
						if(youMove(tmp)) {
							...
							if((!isEnded) && (isTie())) {
								...
								isEnded = true;
								/* You win. Last score incrementing is deleted. Your score increments of 10. */
								setScore(--score);
								setScore(score+10);
								updateViewLabels();
								playTick(endSoundTick);
							...
							if((!isEnded) && (isTie())) {
								...
								isEnded = true;
								/* You lose. Last score incrementing is deleted. Your score decrements of 10. */
								setScore(--score);
								setScore(score-10);
								updateViewLabels();
								playTick(endSoundTick);
